...
using AuthProject.Identity.Helpers;
...
namespace AuthProject.Identity.Services
{
        ...
        public async Task<ApiResponse<string>> RegisterAsync(RegisterRequest request, string origin)
        {
            ...
            if (userWithSameEmail == null)
            {
                ...
                if (result.Succeeded)
                {
                    ...
                    await _emailService.SendAsync(new EmailRequest() { From = "test", To = user.Email, Body = $"Please confirm your account by visiting this URL {verificationUri}", Subject = "Confirm Registration" });
